...
test('it finds the closest points from the cursor', function(assert) {
  ...
  let maxY = max(values.map((d) => {
    let [, ...yValues] = d;
    return max(yValues.sort(ascending));
  }));

  let xScale = scaleLinear().domain(extent(values, (d) => d[0])).range([0, 1000]);
  let yScale = scaleLinear().domain([0, maxY]).range([500, 0]);

  let xActual = xScale(20);
  let yActuals = [10, 20, 30, 40].map(yScale);

  let result = findClosestCursorPoints([xActual + 3, yActuals[1] + 9], [0, 0], xScale, yScale, values);
